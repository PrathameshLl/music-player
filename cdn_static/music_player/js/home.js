...
$("#playlist-post").click(function() {
    ...
    new_options.body = JSON.stringify({ "name": playlist_name });
...
document.querySelector("#add-playlist-song-input").addEventListener("input", (event) => {
    ...
    const search_query = JSON.stringify({ "song_name": event.target.value });
    fetch("music/searchsongs", {
        ...
        .then(response => {
            response.forEach(song => {
                $("#playlist-add-song-result").append(`<li class="px-1 py-1 hover:bg-mycolor-4"> ${song.name} by ${song.artist} </li>`);
            });
        });
});
